...
1. Run `bundle exec jekyll serve -l -H localhost` to generate the HTML and serve it from `localhost:4000` (running it via `bundle exec` ensures Jekyll uses the specific dependency versions from your `Gemfile.lock` rather than whatever is installed globally). The local server will automatically rebuild and refresh the pages on change to Markdown (*.md) and HTML files, while changes to the core template and configuration (i.e., `_config.yml`) will require stopping and restarting Jekyll.
